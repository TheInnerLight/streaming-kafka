...
object KafkaConsumer {
  ...
  /**
    ...
    */
  def commitOffsetsFromQueueEvery[F[_] : Concurrent : Timer : MonadKafkaConsumer[?[_], KafkaConsumerSubscription], K, V](timeBetweenCommits : FiniteDuration)(lockedConsumer : MVar[F, KafkaConsumerSubscription[K, V]])(queue : Queue[F, Map[TopicPartition, OffsetMetadata]]): Stream[F, Unit] =
    for {
      ...
      xs <- Stream
        ...
        .evalMap { case (maybePrevOffsetMap, currentOffsetMap) => {
          ...
          lockedConsumer
            .locked(consumer =>
              MonadKafkaConsumer[F, KafkaConsumerSubscription].commitOffsetMap(commitMap)(consumer)
            )
            .onError { case ex =>
              Sync[F].delay {
  ...
  /**
    ...
    */
  def applyCommitPolicy[F[_] : Concurrent : Timer : MonadKafkaConsumer[?[_], KafkaConsumerSubscription], K, V](consumerVar : MVar[F, KafkaConsumerSubscription[Array[Byte], Array[Byte]]])(config : KafkaConsumerConfig[K, V]) : Pipe[F, ConsumerRecord[Array[Byte], Array[Byte]], ConsumerRecord[Array[Byte], Array[Byte]]] =
    stream =>
  ...
  /**
    ...
    */
  def apply[F[_] : Concurrent : Timer : ContextShift : MonadKafkaConsumer[?[_], KafkaConsumerSubscription], K, V](config : KafkaConsumerConfig[K, V]): Stream[F, Either[Throwable, ConsumerRecord[K, V]]] = {
    ...
    def assignStartingPosition(startingPosition: StartingPosition)(lockedConsumer : MVar[F, KafkaConsumerSubscription[Array[Byte], Array[Byte]]]) =
      lockedConsumer.locked(subscription =>
        for {
          topicPartitions <- MonadKafkaConsumer[F, KafkaConsumerSubscription].topicPartitionAssignments(subscription)
          _ <- startingPosition match {
            case StartingPosition.Beginning     => MonadKafkaConsumer[F, KafkaConsumerSubscription].seekToBeginning(topicPartitions)(subscription)
            case StartingPosition.End           => MonadKafkaConsumer[F, KafkaConsumerSubscription].seekToEnd(topicPartitions)(subscription)
            case StartingPosition.Explicit(pos) => MonadKafkaConsumer[F, KafkaConsumerSubscription].seekTo(pos)(subscription)
    ...
    def pollAndChunk(lockedConsumer : MVar[F, KafkaConsumerSubscription[Array[Byte], Array[Byte]]])(timeout : FiniteDuration): Stream[F, ConsumerRecord[Array[Byte], Array[Byte]]] =
      for {
        records <- Stream.eval(lockedConsumer.locked(consumer => ContextShift[F].evalOn(config.pollingExecutionContext)(MonadKafkaConsumer[F, KafkaConsumerSubscription].poll(config.pollTimeout)(consumer)))).scope
